...
public class GrafoNoDirigido extends Grafo {
    ...
    public LinkedList transformarGrafoSimpleND() {
	...
	while(vertsIt.hasNext()) {
		...
		if(gradoA>1) {
		    ...
		    LinkedList ladosMult = new LinkedList();
		    Set inci = new HashSet();
	
		    inci = this.incidentes(ver);

		    Iterator inciIt = inci.iterator();

		    // almacena en una lista los lados que cubren la misma ruta
		    while (inciIt.hasNext()){
			String nombre = (String) inciIt.next();
		      	if ( this.incidentes(ady).contains(nombre) )
			    ladosMult.add(nombre);
		    }

		    Iterator ladIt = ladosMult.iterator();
		    double menor = 9999999;

		    while(ladIt.hasNext()){
			String side = (String) ladIt.next();
			Lado lado = getLado(side);
			if (lado.peso <= menor)
			    menor = lado.peso;
		    }

		    Iterator ladIt2 = ladosMult.iterator();

		    while(ladIt2.hasNext()){
			String side2 = (String) ladIt2.next();
			Lado lado2 = getLado(side2);
			if (lado2.peso > menor)
			    this.eliminarArista(side2);
		    }	
		}
	    }
	}
...
public class GrafoDirigido extends Grafo {
	...
	while(vertsIt.hasNext()) {
		...
		if(gradoA>1) {
		    ...
		    while (inciIt.hasNext()){
			String nombre = (String) inciIt.next();
		      	if ( this.incidentes(suc).contains(nombre) )
			    ladosMult.add(nombre);
		    }
